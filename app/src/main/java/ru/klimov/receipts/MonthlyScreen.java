...
final class MonthlyScreen {
 ...
 void render(){
  ...
  for(int i=0;i<3;i++){final int n=i;LinearLayout tab=column();tab.setGravity(Gravity.CENTER);int tint=i==a.screenTab?a.color(3):muted();Icon icon=new Icon(a,i,tint);tab.addView(icon,new LinearLayout.LayoutParams(dp(25),dp(26)));gap(tab,4);TextView label=text(tabs[i],12,i==a.screenTab);label.setTextColor(tint);tab.addView(label);tab.setOnClickListener(v->{a.screenTab=n;render();});tab.setContentDescription(tabs[i]);tab.setFocusable(true);bottom.addView(tab,new LinearLayout.LayoutParams(0,dp(58),1));}
